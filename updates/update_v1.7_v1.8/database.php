...
if (isset($_POST["btn_admin"])) {
    ...
    if (mysqli_connect_errno()) {
        ...
        mysqli_query($connection, "ALTER TABLE `payment` ADD `billing_type` VARCHAR(255) NULL DEFAULT NULL AFTER `package`, ADD `payment_type` VARCHAR(255) NULL DEFAULT NULL AFTER `billing_type`, ADD `txn_id` VARCHAR(255) NULL DEFAULT NULL AFTER `payment_type`;");

        mysqli_query($connection, "ALTER TABLE `payment` ADD `package_id` INT NULL DEFAULT '2' AFTER `package`;");

        mysqli_query($connection, "ALTER TABLE `settings` ADD `paypal_mode` VARCHAR(255) NULL DEFAULT 'sandbox' AFTER `paypal_email`;");
        mysqli_query($connection, "ALTER TABLE `settings` ADD `enable_faq` INT NULL DEFAULT '1' AFTER `enable_blog`;");
        mysqli_query($connection, "ALTER TABLE `settings` ADD `paypal_payment` INT NULL DEFAULT '0' AFTER `paypal_email`, ADD `stripe_payment` INT NULL DEFAULT '0' AFTER `paypal_payment`, ADD `publish_key` VARCHAR(255) NULL DEFAULT NULL AFTER `stripe_payment`, ADD `secret_key` VARCHAR(255) NULL DEFAULT NULL AFTER `publish_key`; ");


        mysqli_query($connection, "ALTER TABLE `users` ADD `enable_experience` INT NULL DEFAULT '1' AFTER `enable_followers`, ADD `enable_skill` INT NULL DEFAULT '1' AFTER `enable_experience`, ADD `enable_testimonial` INT NULL DEFAULT '1' AFTER `enable_skill`, ADD `enable_contact` INT NULL DEFAULT '1' AFTER `enable_testimonial`;");

        mysqli_query($connection, "ALTER TABLE `users` ADD `enable_vcard` INT NULL DEFAULT '1' AFTER `enable_contact`;");
        mysqli_query($connection, "ALTER TABLE `users` ADD `appointment_info` TEXT NULL DEFAULT NULL AFTER `cv`;");



        mysqli_query($connection, "DROP TABLE package;");
        mysqli_query($connection, "DROP TABLE features;");


        $query = '';
          $sqlScript = file('sql/package.sql');
        ...
        $query = '';
          $sqlScript = file('sql/features.sql');
          foreach ($sqlScript as $line) {
        ...
        $query = '';
          $sqlScript = file('sql/feature_assaign.sql');
          foreach ($sqlScript as $line) {
